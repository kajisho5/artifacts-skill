    ...
    def inspect(self, ref: ArtifactRef) -> InspectionReport:
        ...
        for elem in root.iter():
            ...
            if kind == "external":
                ...
                # shared with the HTML and Markdown adapters - see
                # reference_resolution.py for why only this resolve/escape/
                # exists step (not URL classification, which differs per
                # format) was worth extracting.
                outcome = resolve_local_reference(svg_dir, url)
                if outcome.escaped:
