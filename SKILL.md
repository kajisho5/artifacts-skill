...
**PNG/JPEG/WebP, HTML, SVG, CSV, Markdown, EPUB, and video/audio media
(MP4/MOV/WebM/WAV).** Every format from the original design brief's Tier 1
and Tier 2 is built, plus CSV/Markdown/EPUB/Media added beyond it. A
genuinely unrecognized file returns a clear `ARTIFACT_TYPE_UNSUPPORTED`
error, never a silent no-op. HTML, SVG, CSV, Markdown, and Media have no
mutating operations by design (inspect/render/verify only — editing/
generating media is squarely another tool's job, not this skill's). EPUB
has one operation (`metadata_set`, title/author only) and renders one PNG
per spine document via Playwright/Chromium (needs `playwright install
chromium`); Media renders one representative frame via `ffmpeg`/`ffprobe`
(needs both on PATH) — see `docs/adapters.md` for details.
...
have no mutating operations, same as HTML/SVG. Media (video/audio) also
has no mutating operations — editing/transcoding media is another tool's
job, not this skill's. Run
